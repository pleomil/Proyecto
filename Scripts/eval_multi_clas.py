...
from models import BinaryCRNN, BinaryCNN, MulticlassCRNN, ViTClassifier
import torch
...
# Cargar modelo CNN
crnn_model = MulticlassCRNN().to(device)
crnn_model.load_state_dict(torch.load(r"models\best_model_CRNN_multiclass.pth", map_location=device))
crnn_model.eval()

# Cargar modelo CRNN
vit_model = ViTClassifier().to(device)
vit_model.load_state_dict(torch.load(r"models\best_model_ViT_multiclass.pth", map_location=device))
vit_model.eval()
...
def get_predictions(model, test_loader):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            preds = torch.argmax(outputs, dim=1)  # Convertir logits a 0 o 1
...
def plot_confusion_matrix(y_true, y_pred, model_name):
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", 
                xticklabels=["Arabe", "Español", "Frances"], 
                yticklabels=["Arabe", "Español", "Frances"])
...
y_true_vit, y_pred_vit = get_predictions(vit_model, test_loader)
y_true_crnn, y_pred_crnn = get_predictions(crnn_model, test_loader)

# Graficar matriz de confusión
plot_confusion_matrix(y_true_vit, y_pred_vit, "ViT")
