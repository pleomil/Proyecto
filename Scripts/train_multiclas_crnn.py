...
from models import MulticlassCRNN
import torch
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for batch in progress_bar:
            inputs, labels = batch[0].to(device), batch[1].to(device).long()  # Asegurar etiquetas enteras
            inputs = inputs.squeeze(2)

            optimizer.zero_grad()
            outputs = model(inputs)  # Logits sin softmax

            loss = criterion(outputs, labels)  # `CrossEntropyLoss` maneja internamente softmax
            loss.backward()
            optimizer.step()

            train_loss += loss.item()

            # Accuracy: Obtener la clase con mayor probabilidad
        ...
        with torch.no_grad():
            for batch in val_loader:
                inputs, labels = batch[0].to(device), batch[1].to(device).long()
                inputs = inputs.squeeze(2)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/best_model_CRNN_multiclass.pth")
